...
# load phenotype data
load("GTEX_phenotype_annotated_SNX19_plusGenotype.rda")

#########################
#### GENE COUNTS ########

### gene count files
geneFn = paste0("/dcl01/lieber/ajaffe/Brain/SNX19/GTEX/Counts/Gene/",
	pd$sra_accession, "_SNX19_Ensembl75_genes.counts")
names(geneFn) = pd$sra_accession
all(file.exists(geneFn)) # TRUE
...
geneCounts = geneCounts[,pd$sra_accession] # put in order
...
colnames(geneStats) = pd$sra_accession
...
### exon count files
exonFn = paste0("/dcl01/lieber/ajaffe/Brain/SNX19/GTEX/Counts/Exon/",
	pd$sra_accession, "_SNX19_Ensembl75_exons.counts")
names(exonFn) = pd$sra_accession
all(file.exists(exonFn)) # TRUE
...
exonCounts = exonCounts[,pd$sra_accession] # put in order
...
bamFiles = pd$bamFile
names(bamFiles)=pd$sra_accession
...
jCounts = as.data.frame(jCounts[names(jMap),pd$sra_accession])
...
save(pd, jMap, jCounts, geneCounts, geneMap, exonCounts, exonMap, 
	...
	file="rawAndRpkmCounts_plusGenotype_SNX19_GTEX_n8206.rda")
