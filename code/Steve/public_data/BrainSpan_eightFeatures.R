...
#load brainspan
load('/dcl01/lieber/ajaffe/lab/brainspan_analysis/rpkmCounts_full_brainspan_n607.rda')
load('/dcl01/lieber/ajaffe/lab/brainspan_analysis/full.brainspan_final.phenotype.rda')
pd=fin_pdSpan
...
all_3_features_gathered <- cbind(all_3_features_gathered, pd[match(all_3_features_gathered$Sample,paste0(pd$Braincode,".",pd$Regioncode)),] )
...
all_3_features_gathered=subset(all_3_features_gathered, !is.na(all_3_features_gathered$Regioncode) )
 pdf('/dcl01/lieber/ajaffe/Steve/SNX19/brainspan/plots/SNX19_8feature_brainSpan_boxplots.pdf')
	 for (k in unique(all_3_features_gathered$Rowname) ) {
	 dat2 = subset(all_3_features_gathered, Rowname==k)
	  p <- ggplot(dat2, aes(x=Regioncode, y=log2(Rpkm+1)) ) +
			geom_boxplot(outlier.colour = NA, alpha = 0) + 
		geom_jitter(alpha = 1, position = position_jitter(width = 0.2, height = 0), aes(col=Age ) ) +
			labs(x = "Region", 
				 y = "log2(RPKM+1)",
...
## pheatmap 
library(pheatmap)
pd$SampleID=paste0(pd$Braincode,".",pd$Regioncode)
all_3_features = all_3_features[,colnames(all_3_features)%in%pd$SampleID]
ann = pd[match(colnames(all_3_features),pd$SampleID ),c('Braincode','Regioncode','Age')]
ann$Braincode = factor(ann$Braincode)
ann$Regioncode=factor(ann$Regioncode)
pdf('/dcl01/lieber/ajaffe/Steve/SNX19/brainspan/plots/SNX19_8feature_brainSpan_heatmap.pdf')
pheatmap(t(scale(as.matrix(all_3_features), center = TRUE, scale = TRUE) ), annotation_col=ann)
dev.off()

## pheatmap 
a=subset(all_3_features_gathered,Rowname=="ENSG00000120451")[,c('Rpkm','Regioncode','Age')]
tidyr::spread(a,Age,Rpkm)
tidyr::spread(all_3_features_gathered[,c('Rowname','Rpkm','Regioncode','Age')])