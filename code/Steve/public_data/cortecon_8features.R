...
all_3_features_gathered <- all_3_features
...
all_3_features_gathered <- cbind(all_3_features_gathered, pd[match(all_3_features_gathered$Sample,pd$SRR),] )
...
all_3_features_gathered$Stage = factor(all_3_features_gathered$Stage,levels=c("Pluripotency","Neural Differentation","Cortical Specification","Deep Layer Formation","Upper Layer Formation" ) )
 pdf('/dcl01/lieber/ajaffe/Steve/SNX19/cortecon/plots/SNX19_8feature_cortecon_timecourse.pdf')
	 for (k in unique(all_3_features_gathered$Rowname) ) {
	 dat2 = subset(all_3_features_gathered, Rowname==k)
	  p <- ggplot(dat2, aes(x=as.numeric(Day), y=log2(Rpkm+1),col=Stage ) )  +
