## 
mediationPairs = openxlsx::read.xlsx('/dcl01/lieber/ajaffe/Steve/SNX19/SNX19-225-meqtls-to-use-for-mediation-032818-cur.xlsx')
mediationPairs = expand.grid(SNP = unique(mediationPairs$SNP),ProbeID = unique(mediationPairs$ProbeID) )
############### DLPFC POLYA ################# 
...
######
mediationPairsFull = data.frame(SNP = rep(as.character(mediationPairs$SNP),each=10), 
						    CpG = rep(as.character(mediationPairs$ProbeID),each=10),
							Feature = rep(as.character(foi$Feature),times=nrow(mediationPairs) ),stringsAsFactors=FALSE )

mediationPairsFull$SNP = snpMapAll[match(mediationPairsFull$SNP, snpMapAll$name ), 'SNP'] 
getMediationStats = function(row_i) {

## subset to features of interest
snp_i = mediationPairsFull[row_i, 'SNP' ]
cpg_i = methMat[mediationPairsFull$CpG[row_i], ]
feature_i = mediationPairsFull[row_i, 'Feature']

keepSNP = !is.na(snpAll[snp_i,])

##### Calculate a: effect of SNP on methylation
mod_a = cbind(mod, pca_meth, t(snpAll[snp_i,])  )
dat_a = data.frame(cpg = cpg_i[keepSNP], mod_a[keepSNP,-1] )
colnames(dat_a)[ncol(dat_a)] <- c(snp_i)

fastResA=summary(lm(cpg~ ., data=dat_a) )
...
dat_b = data.frame(cpg = cpg_i[keepSNP], mod_b[keepSNP,-1], t(yExprs_DlpfcPolyA[feature_i,keepSNP]) )
...
fastResB = fastResB[fastResB$coef=='cpg',]
...
dat_b0 = data.frame(cpg = cpg_i[keepSNP], mod_b0[keepSNP,-1], t(yExprs_DlpfcPolyA[feature_i,keepSNP]) )
...
fastResB0 = fastResB0[fastResB0$coef=='cpg',]

##
res = data.frame(SNP=snp_i,
		CpG=mediationPairsFull$CpG[row_i],
...
mediationStats = lapply(1:nrow(mediationPairsFull), getMediationStats)
mediationStats = data.table::rbindlist(mediationStats)
write.csv(as.data.frame(mediationStats), file='/dcl01/lieber/ajaffe/Steve/SNX19/tables/mediation_stats_dlpfc_890pairs_10features_03_28_18.csv',row.names=F )
######################## Now with rs10791097
getMediationStats = function(row_i) {
