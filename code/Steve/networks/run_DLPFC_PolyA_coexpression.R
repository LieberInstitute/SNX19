#qsub -l bluejay -l mf=45G,h_vmem=55G,h_stack=256M -M stephensemick@gmail.com -cwd -b y R CMD BATCH --no-save run_DLPFC_PolyA_coexpression.R
### 
...
pd$ageGroup = cut(pd$Age, c(-1,0,13,100))

############
# filter
gIndex=which(rowMeans(geneRpkm)>0.05)
...
jIndex=which(rowMeans(jRpkm)>0.5 & jMap$code != "Novel")
jRpkm = jRpkm[jIndex,]
jMap = jMap[jIndex]
...
gIndexes$AA = which(pd2$Race == "AA") #eQTL analysis for African Americans
...
save(outStats, file = "rdas/Multivariate_Coexpression_SNX19_PolyA_FullStats.rda" )
write.csv(outStats, file=gzfile("tables/Multivariate_Coexpression_SNX19_PolyA_FullStats.csv.gz"),
	row.names=FALSE)
table(rowSums(outStats[,grep("pval", colnames(outStats))] < 0.01) )

##Save outStats post-filtered
pval_filt = rowSums(outStats[,grepl("pval",colnames(outStats)) ]<1e-5) >0
write.csv(outStats[pval_filt, ],row.names=FALSE, file = '/dcl01/lieber/ajaffe/Steve/SNX19/coexpression/DLPFC_polyA/tables/Multivariate_Coexpression_SNX19_PolyA_FullStats_1e-5_pvalFilt.csv' )